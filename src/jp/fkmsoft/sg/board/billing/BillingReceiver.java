...
public class BillingReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (BillingLogger.isDebugEnabled()) {
            BillingLogger.info(TAG, "receive message");
        }
        String action = intent.getAction();
        if (MarketAction.ACTION_PURCHASE_STATE_CHANGED.equals(action)) {
        ...
        } else if (MarketAction.ACTION_NOTIFY.equals(action)) {
            String notifyId = intent.getStringExtra(MarketExtra.NOTIFICATION_ID);
            if (BillingLogger.isDebugEnabled()) {
                BillingLogger.info(TAG, "notifyId: " + notifyId);
            }
    ...
    private void purchaseStateChanged(Context context, String signedData, String signature) {
        Intent intent = new Intent(MarketAction.ACTION_PURCHASE_STATE_CHANGED);
        intent.setClass(context, BillingService.class);
        intent.putExtra(MarketExtra.INAPP_SIGNED_DATA, signedData);
        intent.putExtra(MarketExtra.INAPP_SIGNATURE, signature);
    ...
    private void notify(Context context, String notifyId) {
        Intent intent = new Intent(BillingService.Action.ACTION_GET_PURCHASE_INFORMATION);
        intent.setClass(context, BillingService.class);
        intent.putExtra(MarketExtra.NOTIFICATION_ID, notifyId);
        context.startService(intent);
    }
    ...
    private void checkResponseCode(Context context, long requestId, int responseCodeIndex) {
        Intent intent = new Intent(MarketAction.ACTION_RESPONSE_CODE);
        intent.setClass(context, BillingService.class);
        intent.putExtra(MarketExtra.INAPP_REQUEST_ID, requestId);
        intent.putExtra(MarketExtra.INAPP_RESPONSE_CODE, responseCodeIndex);
        context.startService(intent);
    }
}
